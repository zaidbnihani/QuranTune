...
@Database(entities = [QuranCard::class], version = 5, exportSchema = false)
abstract class QuranDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private fun migrateTable(db: androidx.sqlite.db.SupportSQLiteDatabase) {
            db.execSQL("CREATE TABLE IF NOT EXISTS `quran_cards_new` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `title` TEXT NOT NULL, `clipboardText` TEXT NOT NULL, `imageUri` TEXT, `presetResName` TEXT, `reciterIdentifier` TEXT, `notificationTriggerWord` TEXT, `youtubeUrl` TEXT, `sortOrder` INTEGER NOT NULL DEFAULT 0, `timestamp` INTEGER NOT NULL DEFAULT 0, `scheduledHour` INTEGER, `scheduledPeriod` TEXT)")
            ...
            val validCols = setOf("id", "title", "clipboardText", "imageUri", "presetResName", "reciterIdentifier", "notificationTriggerWord", "youtubeUrl", "sortOrder", "timestamp", "scheduledHour", "scheduledPeriod")
